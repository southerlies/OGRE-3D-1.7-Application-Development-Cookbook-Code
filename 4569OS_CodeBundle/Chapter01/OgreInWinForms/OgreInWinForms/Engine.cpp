	...
	: m_Root(NULL)
{
	...
	char ApplicationName[] = "OgreInWinForms.exe";
	...
	m_Root = new Ogre::Root("", "", Ogre::String(ApplicationPath + Ogre::String("OgreInWinForms.log"))); 
	...
	m_SceneManager = m_Root->createSceneManager(Ogre::ST_GENERIC, "WinFormsOgre");
	...
	Ogre::RenderTarget *RenderWindow = m_Root->getRenderTarget("Ogre in WinForms");

	if (RenderWindow == NULL)
	{
	try
	{
		m_RenderWindow = m_Root->createRenderWindow("Ogre in WinForms", rect.right - rect.left, rect.bottom - rect.top, false, &parms);
	}
    catch(...)
	{
