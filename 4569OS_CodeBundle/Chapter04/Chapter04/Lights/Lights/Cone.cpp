...
Ogre::ManualObject * CCone::CreateCone(Ogre::Real Intensity, const Ogre::String& name, const Ogre::String& materialName, Ogre::Real fadeAmount)
{
	Ogre::ManualObject *Cone = NULL;
	Ogre::SceneManager *SceneManager = Ogre::Root::getSingleton().getSceneManager("Lights");
	Cone = SceneManager->createManualObject(name);
	Cone->setDynamic(false);
    Cone->begin(materialName, Ogre::RenderOperation::OT_TRIANGLE_LIST);
	...
	for (int BaseSegmentIndex=0; BaseSegmentIndex <= m_BaseSegments; BaseSegmentIndex++)
	{
		...
		Cone->colour(Intensity, Intensity, 0.0, 0.01);
		Cone->normal(Ogre::Vector3::NEGATIVE_UNIT_Y);
